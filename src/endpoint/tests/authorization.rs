...
impl AuthorizationSetup {
    ...
    fn test_silent_error(&mut self, request: CraftedRequest) {
        ...
        .execute(request)
        {
            Ok(ref resp) if resp.location.is_some() => {
                panic!("Redirect without client id {:?}", resp)
            }
    ...
    {
        let response =
            authorization_flow(&mut self.registrar, &mut self.authorizer, &mut pagehandler)
                .execute(request);
...
fn auth_request_error_malformed_scope() {
    ...
    AuthorizationSetup::new()
        .test_error_redirect(malformed_scope, Allow(EXAMPLE_OWNER_ID.to_string()));
}
