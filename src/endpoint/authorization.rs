use code_grant::authorization::{
    authorization_code, Endpoint as AuthorizationEndpoint, Error as AuthorizationError, Extension,
    Pending, Request as AuthorizationRequest,
};
...
impl<'a, E: Endpoint<R>, R: WebRequest> AuthorizationPending<'a, E, R> {
    ...
    fn convert_result(
        result: Result<Url, AuthorizationError>,
        endpoint: &mut E,
        request: &mut R,
    ) -> Result<R::Response, E::Error> {
        match result {
