...
impl Pbkdf2 {
    ...
    pub fn set_relative_strength(&mut self, strength: u8) {
        assert!(
            strength < 32,
            "Strength value out of range (0-31): {}",
            strength
        );
        unsafe {
            self.iterations = NonZeroU32::new_unchecked(1u32 << strength);
        }
    }
...
impl PasswordPolicy for Pbkdf2 {
    fn store(&self, client_id: &str, passphrase: &[u8]) -> Vec<u8> {
        ...
        output.append(&mut self.salt(client_id.as_bytes()));
        {
            ...
            pbkdf2::derive(&digest::SHA256, self.iterations, salt, passphrase, output);
        }
        output
    }
...
impl Extend<Client> for ClientMap {
    ...
    {
        iter.into_iter()
            .for_each(|client| self.register_client(client))
    }
}
